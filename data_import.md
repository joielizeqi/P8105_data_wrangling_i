...
Importing FAS_pups.csv using absolute path. Absolute paths are fragile
and not sustainable between collaborators.
...
If data contains irrelevant info in the first few rows, use the `skip`
argument.
...
Use the `na` argument to replace all missing data and make sure data are
numerical
